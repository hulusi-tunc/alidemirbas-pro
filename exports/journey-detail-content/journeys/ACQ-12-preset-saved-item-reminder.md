## ACQ-12 — Abandoned Selection Recovery (preset: Saved Item Reminder)

- Canonical ID: `ACQ-12`
- Slug: `abandoned-selection-recovery`
- Preset ID: `saved-item-reminder`
...
- Source route: `/lab/customer-journeys/saved-item-reminder`
...
**Preset · Saved Item Reminder**
- canonical rule: The selection is a saved list or wishlist: a declared interest with no purchase intent asserted, so the first check is much later and a change on a saved item is the honest reason to write.
- sets: selection.first_check = 3 days–7 days · selection.lifetime = 14 days–30 days
...
### Configure
- `selection.first_check` [recommended default] (recovery-window): 3 days–7 days (preset override; canonical default: 1 hour–4 hours) — The first check waits long enough after the last selection activity that the person has actually left rather than paused, and no longer than the selection is likely to be remembered.
...
- `selection.lifetime` [recommended default] (recovery-window): 14 days–30 days (preset override; canonical default: 7 days–14 days) — The recovery lifetime is the period in which a held selection is still an intent rather than a record; past it nothing further is sent.
