...
how does its result reliably reach back to the lifecycle system that depended on it?

## POST-REPAIR UPDATE (2026-09-04)

The audit below (unchanged, preserved as the historical record of what was found) was followed by
a **repair round** that closed every one of the 11 confirmed P0s directly in `src/canonical/*.ts`,
added 2 new regression validators to `scripts/validate-canonical.mjs`, and regenerated all 124
contracts from the corrected source. Full detail: `FIXES-APPLIED.md` (every P0's before/root
cause/after/repair layer/validator/regression test), `CANONICAL-CHANGES.md` (the 4 canonical graph
topology changes: `DAT-228`, `CTL-232`, `INC-258`, `DEC-181`), `VALIDATOR-COVERAGE.md` (the 2 new
validators).

**Headline change: P0 11 → 0. `NEEDS_CANONICAL_CHANGE` 3 → 0.** Readiness distribution moved from
0 `READY` / 88 `READY_WITH_MAPPING` / 33 `NEEDS_CONTRACT_WORK` / 3 `NEEDS_CANONICAL_CHANGE` to
0 / **98** / **26** / **0** (all 124 re-derived from the corrected source, not copied forward).
P1 moved 75 → 74 and P2 100 → 99 as a byproduct of gap removal/consolidation on the repaired
workflows (not a target — P1/P2 were deliberately left non-zero; `READY_WITH_MAPPING` is the
expected healthy end state, not `READY`). The "Top 20 findings," "Recurring patterns," and
"Domain-cluster quality" sections below are left as originally written (they are the accurate
record of what this round *found*) except for the numeric tables directly restated in this update
section and the domain-cluster table, which now reflects the repaired counts. Every finding
numbered 1–6 below describing a P0 is now fixed — see `FIXES-APPLIED.md` for the repair applied to
each.

Nothing in `src/canonical/*.ts` was touched *casually*: every canonical-graph change is documented
with an explicit "why metadata was insufficient" justification in `CANONICAL-CHANGES.md`, and no
closed layer (Customer Journeys, Silent Lifecycle States, Runtime Mechanisms) was modified — the
one candidate cross-layer touch, `OPS-131` as a `DEC-181` sender, was verified to need no change
and was left alone.
...
**Pre-repair (as originally audited):**
...
**Post-repair (current, 2026-09-04):**

| Verdict | Count | % |
|---|---|---|
| READY | 0 | 0% |
| READY_WITH_MAPPING | 98 | 79% |
| NEEDS_CONTRACT_WORK | 26 | 21% |
...
`READY_WITH_MAPPING` is for, and 98 of 124 (79%) now reach it cleanly, up from 88 before the repair
round (the 11 repaired P0 workflows plus one workflow whose `NEEDS_CONTRACT_WORK` gap was resolved
as a byproduct of a shared handoff fix all moved up).
...
totals line, generated by the same script, re-derived from the corrected source, not copied
forward.) All 11 P0s were repaired at the smallest correct layer — 7 as implementation-contract
fixes (idempotency keys, authority declarations, ownership branches) and 4 as canonical graph
topology changes (`DAT-228`, `CTL-232`, `INC-258`, `DEC-181`) — see `FIXES-APPLIED.md` for the
full register and `CANONICAL-CHANGES.md` for the graph changes specifically. P1 and P2 were
deliberately **not** driven to zero: `READY_WITH_MAPPING` — not `READY` — is this program's
defined healthy end state, and the remaining 173 findings are genuine mapping-time policy
dependencies (SLA values, team/queue names, approval thresholds) the canonical source correctly
leaves undeclared rather than invents.
...
1. **`DEC-181` (Decision Request), the corpus's single general escalation sink, has an entry-
   ...
   carry.
   *[FIXED, 2026-09-04 — DEC-181; see `FIXES-APPLIED.md`.]*
...
2. **Two genuine concurrent-duplicate-effect P0s**, both confirmed against raw source:
   ...
   consumed.
   *[FIXED, 2026-09-04 — RSK-192 / RSK-198; see `FIXES-APPLIED.md`.]*
...
3. **`SUB-164` (Renewal Execution)** has no `idempotencyKey`/`attemptBudget` on the actions that
   ...
   *[FIXED, 2026-09-04 — SUB-164; see `FIXES-APPLIED.md`.]*
...
4. **`OWN-56` (Approval Request) never addresses self-approval**, even though its own stated
   ...
   *[FIXED, 2026-09-04 — OWN-56; see `FIXES-APPLIED.md`.]*
...
5. **Three genuine canonical-graph defects, not contract gaps** — the round's own
   ...
   *[FIXED, 2026-09-04 — INC-258 / CTL-232 / DAT-228; see `FIXES-APPLIED.md`.]*
...
6. **Undefined decision authority for irreversible choices recurs across domains**: `RLT-247`
   ...
   cluster generally.
   *[FIXED, 2026-09-04 — RLT-247 / TRM-101; see `FIXES-APPLIED.md`.]*
...
Post-repair (current, 2026-09-04 — see the pre-repair table preserved in git history / the
original commit for this round's audit-only baseline):
...
| decision | 9 | 6 | 3 | 0 | 0 | 6 |
| rollout | 10 | 6 | 4 | 0 | 0 | 8 |
| subscription | 1 | 1 | 0 | 0 | 0 | 0 |
| scheduling | 1 | 1 | 0 | 0 | 0 | 0 |
| control | 8 | 5 | 3 | 0 | 0 | 3 |
| terminal | 6 | 3 | 3 | 0 | 0 | 9 |
| incident | 9 | 5 | 4 | 0 | 0 | 6 |

Every `NEEDS_CANONICAL_CHANGE` and every P0 cell is now 0 in every domain. `ownership` (OWN-56)
and `risk`/`data`/`decision`/`rollout`/`control`/`incident`/`subscription` (the other 10 P0
workflows) each moved their fixed workflow(s) from `NEEDS_CONTRACT_WORK`/`NEEDS_CANONICAL_CHANGE`
into `READY_WITH_MAPPING`. `financial`, `access`, `integration`, and `time` remain the round's
cleanest domains (zero-or-near-zero findings above P2). `terminal`, `rollout`, `decision`,
`document`, and `incident` still carry most of the round's remaining P1/P2 work, concentrated in
genuine mapping-time policy dependencies rather than production blockers.
...
WHY:
The 'proposal without disturbance' discipline was already excellent, and the one real gap — one of two paths into CTL-234 skipping revalidation — is now closed by mirroring the pattern this workflow's own sibling CTL-233 already uses, scaled down to this workflow's needs. entity now declares instanceKey: ["transfer_request_id"] and concurrency: "one-active-per-key", plus a note stating explicitly that both paths into CTL-234 revalidate immediately before handing off, because CTL-234's own entry contract requires a transfer revalidated against current state and nothing upstream can guarantee state hasn't moved between initial validation and execution. c.acceptance's 'It is not [required]' branch no longer goes straight to h.execute: it now routes to a new a.revalidate node (idempotencyKey "transfer_request_id + a.revalidate"), which re-reads the same authority/eligibility/transferability facts c.authority/c.eligible/c.transferable originally checked, then a new c.revalidated condition with two branches — 'It holds' → h.execute (rejoining the original flow) and 'It no longer holds' → the existing a.reject node, reused rather than duplicated. Only two nodes were added by deliberately reusing a.reject as the shared failure target instead of building a parallel reject path.
...
INSTANCE:
Scope = the entity, current owner, proposed owner, and the transfer request between them. entity now declares instanceKey: ["transfer_request_id"] and concurrency: "one-active-per-key"; entity.note is still explicit the request is a proposal and the current owner is unaffected by its mere existence, and now additionally states both paths into CTL-234 revalidate immediately beforehand — the acceptance-required path via CTL-233's own a.revalidate, the acceptance-not-required path via this workflow's own new a.revalidate.

ENTRY:
Trigger ownership_transfer_requested is unchanged — a direct, explicit proposal request. What changed is downstream of entry, not the trigger itself: the acceptance-not-required branch no longer treats request-time validation as sufficient authorization for an execution that may happen much later.

OWNERSHIP:
a.preserve explicitly records the current owner remains authoritative throughout — avoids the classic 'entity briefly ownerless during a pending transfer' bug. Unchanged by this round's fix.
...
AUTHORITY / APPROVAL:
c.authority, c.eligible, c.transferable are three explicit negative gates (requester's right to initiate, recipient's eligibility, entity's transferability/holds) before a proposal can even become pending — thorough. The same three facts are now re-checked a second time, immediately before execution, by the new a.revalidate on the 'no acceptance required' path — mirroring how CTL-233 re-confirms them on the acceptance-required path — so authorization-for-execution is a fact about the entity now on both paths, not only a fact about the entity at original request time.

IDEMPOTENCY:
Whether two concurrently-pending transfer proposals for the same entity are themselves a problem is still handled acceptably by design — CTL-233 catches staleness/supersession at acceptance time, unchanged. The sharper, narrower defect this round fixed: the 'no acceptance required' branch (c.acceptance → a.revalidate, idempotencyKey "transfer_request_id + a.revalidate") no longer executes on validation that may be stale — a.revalidate/c.revalidated re-checks authority, eligibility and transferability immediately before h.execute, closing the gap where a transfer could cut over against an entity state that had since changed.
...
CANCELLATION / SUPERSESSION:
x.rejected and x.pending both leave the current owner unaffected. The capability to withdraw a pending request is asserted in a.pending's prose ('the request can be withdrawn by them at any point before it completes') but has no corresponding condition, exit, or handoff node in the graph (confirmed against the raw source) — withdrawal is not a distinguishable, modeled state; this is a separate, still-open finding untouched by the revalidation fix (see gaps). The new c.revalidated 'It no longer holds' branch deliberately reuses the existing a.reject node and x.rejected exit rather than opening a second, parallel rejection outcome.

HANDOFFS:
- h.execute → CTL-234: carries ["the entity, the current owner, the proposed owner and the transfer scope","the revalidation that supported it","the explicit fact that ownership has not moved - the cutover establishes the new owner before touching the old one"]
...
OBSERVABILITY / AUDIT:
Capture, authority/eligibility/transferability checks, revalidation on both paths into CTL-234, and pending/rejected outcomes are each logged.
...
TEST CASES:
- [stale-work] Given: A transfer that does not require recipient acceptance is validated, and the entity's state changes before h.execute would fire → Expect: a.revalidate re-checks authority, eligibility and transferability immediately before execution; c.revalidated's 'It no longer holds' branch routes to the existing a.reject node rather than letting stale validation reach CTL-234 — it must not act from stale state
- [completion] Given: A transfer that does not require recipient acceptance is validated and nothing about the entity's state changes before execution → Expect: c.revalidated's 'It holds' branch rejoins h.execute exactly as before, carrying the same payload plus the fresh revalidation, and hands off to CTL-234 which can honor its own 'revalidated against current state' entry requirement
- [handoff] Given: A transfer that does require recipient acceptance is validated and the proposed owner later accepts → Expect: The acceptance-required path is unchanged — CTL-233 still performs its own a.revalidate at acceptance time and hands off to CTL-234 exactly as before this fix, continuing to work as it always has
- [cancellation] Given: The current owner wants to withdraw a pending transfer request before it completes → Expect: A modeled state (condition/exit/handoff) exists for withdrawal, not just a prose assertion that it is possible — this remains an open P2, unaffected by the revalidation fix
...
WHY:
The authority-switch discipline itself (single-authoritative-system, rollback-defined-before-relying-on-it, timeout-as-success on the observation window) is the strongest-reasoned logic in the batch, and the rollback path now actually consumes what it preserves: a.rollback hands off into a new a.reconcile-preserved action, which mints a rollback_snapshot_id and either rejoins x.rolled-back cleanly or, on a genuine conflict with the source's own independent changes since, hands off to DEC-181. Only pipeline wiring (who performs reconciliation, the observation-window duration) remains.
...
INSTANCE:
Scope is 'the migration, the source system or state, and the target that would replace it'; instanceKey: ["migration_id"], concurrency: "one-active-per-key" — only one cutover can be active for a given migration at a time. 'Exactly one side is authoritative at any moment. Dual authority is the failure this journey exists to prevent, not a transitional convenience' — a strong, explicit single-instance invariant now backed by a formal key.
...
OWNERSHIP:
Fully automated authority-switch mechanics; c.rollback-defined, c.recovery and the new c.reconciled are all system-evaluated decisions, not human ones, though the underlying rollback-window/plan values are presumably set by a human migration owner upstream (out of this workflow's scope).

ASSIGNMENT / QUEUE:
N/A — no human routing declared within this workflow.

DATA:
Cutover boundary (scope/moment/write-control method), write-control state, observed critical-failure signal, rollback plan capability, dependent-state recalculation, and — new — a minted rollback_snapshot_id identifying the preserved cutover-window writes.

EVIDENCE:
N/A — not a review workflow; 'critical failure detected' is a direct system observation, not adjudicated evidence.

AUTHORITY / APPROVAL:
c.rollback-defined explicitly halts to DEC-181 if rollback semantics aren't defined before cutover, rather than proceeding and discovering there's no way back mid-incident — a correct, load-bearing gate. The new c.reconciled adds a second, narrower authority point: a genuine conflict between the preserved rollback-window writes and the source's own independent changes since routes to DEC-181 via h.decide-conflict rather than either side being silently preferred.

IDEMPOTENCY:
a.rollback now carries idempotencyKey "migration_id + a.rollback"; the new a.reconcile-preserved carries idempotencyKey "rollback_snapshot_id + a.reconcile-preserved" — a redelivered rollback or reconciliation job cannot re-mint a second snapshot or double-apply the reconciliation. A genuine declared-field improvement over the rest of this batch, not just prose.
...
ESCALATION:
N/A in the classic sense; c.recovery routes directly to rollback or forward correction based on what has already happened, and c.reconciled routes a genuine conflict to DEC-181 once — neither is an escalation tier or loop.

CANCELLATION / SUPERSESSION:
Rollback explicitly records everything the target accepted during the cutover window 'so nothing written there is simply lost', and that preserved scope is now actually consumed: a.reconcile-preserved reconciles it against the source under rollback_snapshot_id before x.rolled-back is reached, rather than being recorded and left untouched.
...
- h.decide-conflict → DEC-181: carries ["rollback_snapshot_id, the preserved writes it identifies, and the source's own independent changes to the same records","the explicit fact that applying either side blindly discards real work - this is a judgment call, not a data-quality defect"]
...
COMPLETION:
x.authoritative (TARGET_AUTHORITATIVE) is a genuine completion — the point 'at which the old system stops being the answer to anything', explicitly distinguished from mere copy-completion. x.rolled-back is non-terminal, and is now reached only after a.reconcile-preserved has actually run against the preserved writes — its state text was updated from 'recorded for reconciliation' to 'reconciled against it', past tense, because that is now actually true rather than a promise the graph never kept.

RESULT / FEEDBACK:
DAT-227 is the sole real handoff sender (h.cutover); this workflow itself sends onward to DEC-181 (rollback-undefined, or now a genuine reconciliation conflict) or DAT-230 depending on path.

CORRECTION / REOPEN:
Post-stabilization problems are corrected forward against the live target (implied, consistent with the corpus's general reEntry-prose pattern rather than an explicit handoff edge, which is standard elsewhere in this batch and not itself a gap). A rollback that hits a genuine conflict during reconciliation is no longer silently resolved either way — c.reconciled hands it to DEC-181 rather than the workflow choosing which side wins.

OBSERVABILITY / AUDIT:
migration_log appends at boundary/control-writes/switch/rollback/forward/stable, plus the new reconcile-preserved and reconciled steps — good coverage of the switch mechanics and now of the reconciliation that follows a rollback.
...
- [reopen] Given: a rollback is executed after the target accepted cutover-window writes → Expect: the writes are captured under a minted rollback_snapshot_id and a.reconcile-preserved actually reconciles them against the source before x.rolled-back is reached — no longer merely recorded and abandoned
- [escalation] Given: reconciling the preserved cutover-window writes finds a genuine conflict with the source's own independent changes since the switch → Expect: c.reconciled routes to h.decide-conflict→DEC-181 with the snapshot, the preserved writes, and the source's independent changes, rather than either side being silently preferred; a clean reconciliation instead rejoins x.rolled-back directly
- [duplicate-creation] Given: the original migration/operation record that a.rollback and a.reconcile-preserved act against → Expect: is never overwritten by rollback or reconciliation — both only append the preserved writes and the reconciliation outcome to migration_log, keeping the append-only history intact
...
WHY:
The window-bounding and side-effect-suppression discipline is strong and correctly escalates when the suppression list isn't defined. The earlier gap between a.reconcile-current's own stated intent — 'a deliberate correction with consequences somebody has to own — not a side effect of a data load nobody reviewed' — and what the graph modeled is now closed: a.reconcile-current is execution:"human", channel-routed, and its does-text now requires review by an authorized operational role before a currently-authoritative balance, entitlement or status is touched. Only pipeline wiring (which team fills that role) remains.
...
INSTANCE:
Scope is 'the backfill, its defined window, and the target records inside it'; instanceKey: ["backfill_id"], concurrency: "one-active-per-key" — only one backfill can be active against a given gap at a time. 'The window is exact. A backfill without a bounded scope is an unbounded rewrite that nobody can verify closed anything' — a strong bounding discipline now backed by a formal key.

ENTRY:
Trigger historical_gap_identified, authoritative, requires an identified missing window or scope in a historical dataset.

OWNERSHIP:
Mostly automated (define/suppress/load/verify), with one deliberate exception: a.reconcile-current is now execution:"human" and channel-routed, requiring review by an authorized operational role before a backfilled event bearing on current authoritative state (a balance, entitlement, or status) is reconciled. This closes the earlier contradiction between the action's own stated intent and what the graph modeled; the parallel c.side-effects gate continues to escalate to DEC-181 whenever the suppression list is undefined.

ASSIGNMENT / QUEUE:
channels: ["task"] on a.reconcile-current — required because execution:"human" actions must declare a channel, per the pre-existing human_action_without_channel validator rule. The authorized operational role is the queue's implicit owner; no company-specific team is named beyond that abstract role, which is deliberate rather than a gap.
...
AUTHORITY / APPROVAL:
c.side-effects correctly halts to DEC-181 if the suppression list is undefined, before any load happens — good discipline. a.reconcile-current now carries an equivalent gate of its own: review by an authorized operational role is required before any backfilled event bearing on current authoritative state is reconciled, closing the earlier asymmetry between the two judgment points.

IDEMPOTENCY:
a.load is explicitly keyed so re-running the backfill 'fills the same gap rather than doubling it', now backed by idempotencyKey "backfill_id + a.load"; historical timestamps are explicitly preserved rather than restamped. a.reconcile-current carries idempotencyKey "backfill_id + a.reconcile-current", so a redelivered reconciliation review cannot re-apply the same correction twice.

SLA / TIME:
N/A — no wait nodes.

ESCALATION:
Single-hop escalation to DEC-181 for undefined suppression policy; a.reconcile-current's authorized-operational-role review requirement is a review gate on a human action rather than an escalation tier, and does not loop.
...
OBSERVABILITY / AUDIT:
backfill_log (and suppressed_sends) appends at define/suppress/load/reconcile-current/verify — reasonable trail; the reconcile-current step's decision-maker is now identifiable as the authorized operational role required to review it, closing the earlier audit-trail gap.
...
- [correction] Given: a backfilled event bears on current authoritative state (a balance, status, entitlement, or count) → Expect: a.reconcile-current requires review by an authorized operational role before reconciling, decision by decision — matching the action's own stated intent that this is a deliberate correction somebody has to own
- [approval] Given: a.reconcile-current is reached for a backfilled event bearing on current authoritative state → Expect: execution:"human" and channels: ["task"] route it to an authorized operational role for review rather than applying automatically, per the human_action_without_channel validator rule
...
WHY:
The core intake logic (dedupe, validity, deterministic-policy filter, scope statement) is precise and well-guarded, and the entry-semantics gap that used to keep this out of READY has now been closed structurally rather than papered over per caller. DEC-181 is the corpus's general escalation sink — roughly 30 real senders across Customer Journeys, Runtime Mechanisms, and Operational Workflows (OPS-131, CTL-231, CTL-238, REL-95/96/97/99/100, DAT-222/223/226/228/229/230, RSK-191..200, DOC-212..220, INT-115..119, SUB-163..169, FIN-137, REM-152/154/159, TIM-67, and others) — and its intake schema previously assumed every sender was a human/customer submitting a request. The fix generalizes the referring party into two first-class shapes, a human/customer requester or a named internal referral (a Runtime Mechanism or Operational Workflow escalating under its own canonical authority), applied at exactly three points in the graph — a.capture (what is captured), c.valid (what 'valid' means for each shape), and c.info (what happens when required information is missing) — rather than by branching per caller. That is the right shape for a 30-sender shared workflow: one reusable structural distinction, not thirty bespoke paths.
...
INSTANCE:
Scope = the decision request and the business entity/state it concerns. entity now declares instanceKey: ["request_id"] and concurrency: "one-active-per-key", plus an extensive note distinguishing the two referring-party shapes and stating that request_id identifies one request regardless of which shape referred it. Identity for the separate scope-level dedupe question is still enforced procedurally via c.duplicate ('does an open case already cover this scope'), which the new request-level instanceKey does not by itself resolve — see the remaining idempotency gap below.

ENTRY:
Trigger authorized_decision_required is unchanged and remains authoritative and precisely scoped ('an action or state that cannot proceed without an authorized judgment'), explicitly excluding two common false positives (system uncertainty that is really a missing rule; a request being merely unusual). Entry is now equally precise for both referring-party shapes: a.capture's does-text explicitly captures 'the referring party — a human requester, or the referring mechanism/workflow's own id for an internal referral', and c.valid's branches are broadened to validate either a human requester's own standing or, for an internal referral, that it names a real referring mechanism/workflow and is well-formed — explicitly not re-litigating that referrer's own authorization to escalate, which is established under its own canonical rules.

OWNERSHIP:
Unchanged and still clean: this workflow deliberately does not establish review ownership itself — h.assign to DEC-182 carries 'no review has started and no outcome is implied by the case existing', and DEC-182 is where a claimed owner is actually established. What changed is who h.assign identifies as the party behind the case: carries now include 'the referring party — a human requester or the referring mechanism/workflow's own id', so DEC-182 (and any human reviewer) can see who or what is accountable for the request without inferring it.
...
DATA:
a.capture's field list (request id, decision type, target entity, referring party, requested action, decision scope, submission time, required authority, supporting context) is now explicit and well-provenanced for both referring-party shapes: 'referring party' is captured as either a human requester or the referring mechanism/workflow's own id for an internal referral, closing the previous gap where 'requester' and 'requested action' had no natural value for a mechanism-sourced referral.

EVIDENCE:
N/A — this is an intake/triage workflow; evidence evaluation is DEC-183's job. c.valid's target/action/referring-party checks are identity validation, not evidence review.

AUTHORITY / APPROVAL:
N/A here by design, unchanged — c.deterministic explicitly exists to keep policy-decidable actions out of the human-judgment queue in the first place. What is new: for an internal referral, this workflow explicitly does not re-check the referring mechanism/workflow's own authorization to escalate — that authorization was already established under the referrer's own canonical rules — c.valid only confirms the referral itself is well-formed (names a real referring mechanism/workflow). That is a deliberate, stated non-duplication of authority checking across layers, not a gap.

IDEMPOTENCY:
c.duplicate/a.link is still the intended scope-level de-dup guard, and the atomic-claim gap on c.duplicate → a.create it describes is unchanged by this round's fix (see gaps below) — it remains open. What did change: a.capture now carries idempotencyKey "request_id + a.capture", and entity declares instanceKey: ["request_id"] with concurrency: "one-active-per-key", giving request-level (not scope-level) idempotency — request_id is minted on entry, so a retried or duplicated handoff of the same referral is not reprocessed as a second, independent capture.
...
CANCELLATION / SUPERSESSION:
x.lapsed (withdrawn/timed out before a case existed) and x.linked (folded into an existing case) are both correctly non-terminal with honest reEntry semantics; neither is treated as a decision. The new x.returned exit (an internal referral bounced back for missing information) is the same shape — non-terminal, explicit that no case was opened, with its reEntry stating the referrer's own re-escalation is assessed fresh rather than remembering what was incomplete about the prior attempt.

HANDOFFS:
- h.invalid → external:requesting-process: carries ["which of the target, the action, a human requester's standing, or an internal referral's own well-formedness failed to hold up","that nobody decided this - it was rejected by the process, and a corrected request (or a corrected referral) is a new request"]
...
- h.assign → DEC-182: carries ["request_id, the case, its explicit scope and the authority the decision requires","the referring party - a human requester or the referring mechanism/workflow's own id","the explicit fact that no review has started and no outcome is implied by the case existing"]
...
CONSUMER COVERAGE:
Classification: active. Extremely heavy caller list (~30 real senders) confirms this is the corpus's genuine general escalation sink, matching its load-bearing role. The callers were never homogeneous — some (DEC-188's h.renew, CTL-231's h.review) carry a natural named requester and fit a human/customer intake model cleanly, while others (OPS-131, CTL-238, most DAT-2xx/RSK-1xx) are governance/mechanism referrals with no requester at all — but that heterogeneity is now handled by the workflow's own generalized two-shape referral model rather than being a mapping gap. OPS-131 in particular was verified to need no changes of its own: its existing h.escalate carries (exclusion_group, scope_instance_id, contenders, precedence text) map cleanly onto DEC-181's generalized model without needing to carry request_id, since request_id is self-minted downstream by DEC-181's own a.capture.
...
- [handoff] Given: A human requester submits a decision request with their own standing to ask → Expect: a.capture records them as the referring party and c.valid checks their standing directly — the human/customer path is unchanged by this fix
- [handoff] Given: A customer-originated request arrives through the same intake → Expect: Treated identically to a human requester under the same 'human/customer requester' shape — no separate branch is needed, since both are one first-class origin, not two
- [handoff] Given: A Runtime Mechanism (e.g. a DAT-2xx or RSK-1xx workflow) refers a decision it cannot make itself → Expect: a.capture records the referring mechanism's own id as the referring party; c.valid confirms the referral names a real mechanism and is well-formed, without re-checking that mechanism's own authorization to escalate
- [handoff] Given: OPS-131 hands off a genuine precedence-tie arbitration (exclusion_group, scope_instance_id, contenders, precedence text) as an internal referral → Expect: DEC-181's generalized referral model accepts it without OPS-131 needing any change of its own — request_id is minted downstream by DEC-181's own a.capture, so OPS-131's existing h.escalate carries map cleanly
- [duplicate-creation] Given: The same referral is submitted twice (e.g. a retried handoff) carrying the same request_id → Expect: idempotencyKey "request_id + a.capture" plus entity concurrency "one-active-per-key" prevent the retry from being processed as a second, independent capture
- [handoff] Given: An internal referral names a mechanism/workflow id that does not correspond to a real sender, or a human requester's standing does not hold up → Expect: c.valid's broadened 'Invalid' branch catches either shape and routes to a.invalid/h.invalid — no unauthorized origin, human or internal, reaches c.deterministic or opens a case
- [handoff] Given: Any of the ~30 senders hands off into DEC-181 → Expect: The receiving instance's identity (request_id) is self-minted by a.capture on entry rather than expected from the sender — this is why senders were not required to carry a field DEC-181 itself creates, documented for the ~9 affected senders in production/vnext-warning-reviews.json
- [missing-evidence] Given: An internal referral (Runtime Mechanism or Operational Workflow) is judged incomplete by c.info → Expect: a.return-to-referrer records what was missing and returns it via x.returned — no interactive wait is opened, since there is no human on the other end to prompt
- [missing-evidence] Given: A human/customer-originated request is judged incomplete by c.info → Expect: a.pending-info records PENDING_INFORMATION and w.info waits for the requester to supply it or withdraw — unchanged from before, and distinct from the internal-referral path above

GAPS:
- P1 (idempotency): The scope-level dedupe (c.duplicate → a.create) still has no atomic claim: two requests over the same open scope arriving concurrently could both read 'no open case' before either writes one, despite the workflow's own explicit guardrail against exactly that. The new instanceKey/idempotencyKey are keyed on request_id, not on scope, so they do not close this particular race.
- P2 (other): h.invalid (from c.valid's 'Invalid' branch) still targets 'external:requesting-process', a name suited to an external/customer submitter, and was not part of this round's fix; no consumer record shows an internal mechanism/workflow sender receiving or handling that specific bounce-back — unlike c.info's missing-information path, which now does have one (a.return-to-referrer/x.returned) for internal referrals.
...
WHY:
Most of the workflow is well-built (temporary mitigations cannot silently become permanent controls; detached cases are explicitly not closed by the incident closing; obligations and post-incident review are routed rather than dropped). The prior gap — the graph was structurally reachable via INC-252→h.investigate→INC-255→h.recovery→INC-256→h.observe→INC-257→h.resolve→INC-258 without ever passing through INC-253, so an incident could arrive here having applied zero mitigations, and c.mitigations' two branches ('safe to remove' / 'should become permanent') were both silently unsatisfiable in that case — is now fixed in the canonical graph. c.mitigations names all three reachable states: the two existing branches' when clauses were tightened to explicitly require 'at least one temporary mitigation was applied,' and a third branch ('None were ever applied') routes to a new action node, a.no-mitigations. That node rejoins the existing flow at c.cases rather than opening a separate closure route, so a no-mitigation incident receives exactly the same case-handling scrutiny (detached cases, surviving obligations, review-eligibility) as every other outcome — there is no path from 'nothing was ever mitigated' straight to a successful RESOLVED that skips it.
...
INSTANCE:
Work instance is the incident plus every linked case and obligation still attached to it (entity.scope). entity.instanceKey: ["incident_id"] and concurrency: "one-active-per-key" are now declared, alongside a guardrail note explaining that c.mitigations names all three reachable states.
...
OWNERSHIP:
Not re-declared; presumably still the incident command from INC-252, now closing out. No new ambiguity beyond the residual approver gap noted below.
...
AUTHORITY / APPROVAL:
c.mitigations decides whether a temporary control becomes a permanent formalized change (a.formalize) — a real, semi-irreversible decision with no named approver (see the P1 gap below); this is now the workflow's only remaining authority gap.

IDEMPOTENCY:
a.resolve now carries idempotencyKey: "incident_id + a.resolve" — a redelivered resolution trigger reconciles the same closure record once rather than risking a second closure pass over the same incident.
...
COMPLETION:
RESOLVED is explicitly a statement about the shared cause only — the entity.note and guardrails are unusually clear that entity-level obligations are not closed by this, matching the OPS-130 precedent (technical/systemic completion is not business completion) directly and by name-worthy analogy. The new a.no-mitigations branch reinforces this from the other direction: an incident that never had a mitigation applied still passes through the same c.cases logic before RESOLVED is reachable, so 'nothing was ever fixed' can't quietly present as the same successful closure as every other path.
...
CORRECTION / REOPEN:
reEntry on x.resolved explicitly treats later recurrence as evidence against this closed record rather than silently rewriting it — sound correction semantics for the incident-level record, unaffected by the c.mitigations fix since a.no-mitigations rejoins c.cases rather than adding a second closure/reopen path to reconcile.
...
TEST CASES:
- [completion] Given: an incident reaches closure via the investigation-first path (INC-255→INC-256→INC-257) having never applied any mitigation in INC-253 → Expect: c.mitigations takes its new 'None were ever applied' branch to a.no-mitigations, which rejoins c.cases — the incident is routed through the same case-handling scrutiny as every other outcome and cannot silently reach a successfully-resolved closure without it
...
WHY:
Version-binding, the three-outcome branching, and the never-default-to-yes SLA design are all sound and mapping-only. The self-approval gap is now closed structurally: c.outcome gains an explicit "Self-approval attempted" branch (acting reviewer's identity == this request's own requester) routed to h.rejected before the APPROVED branch can fire, and the APPROVED branch itself now requires reviewer != requester. Reassignment of the request does not erase the original requester identity that this check is bound to.
...
INSTANCE:
entity.scope = "the approval request, bound to one specific version of the subject" -- the version binding IS the identity; an approval "attached to an entity rather than a version authorises whatever that entity becomes afterwards" is explicitly named as the failure mode this avoids. entity.instanceKey = ["request_id"], concurrency "one-active-per-key" so a.create cannot mint two live requests for the same subject version.
...
AUTHORITY / APPROVAL:
Closed. c.outcome now distinguishes requester from approver structurally: a "Self-approval attempted" branch (reviewer identity == requester identity) routes to h.rejected instead of a.approved, and the APPROVED branch's own condition now requires reviewer != requester. A guardrail documents the requirement. Single/multiple approval is correctly deferred to OWN-57. Rejection is terminal for the version (h.rejected -> OWN-59), and h.rejected's carries now cover both the ordinary-rejection and self-approval-blocked cases.

IDEMPOTENCY:
a.create carries idempotencyKey "request_id + a.create" and entity.concurrency "one-active-per-key" on instanceKey ["request_id"] together prevent two live PENDING_REVIEW requests for the same subject version. a.approved carries its own idempotencyKey "request_id + a.approved".
...
- h.rejected → OWN-59: carries ["the rejection reason - a content rejection or, for a blocked self-approval attempt, that no eligible review occurred at all","the exact version rejected, which must not later execute"]
...
TEST CASES:
- [self-approval] Given: The requester of an approval-required request is also the only identity that attempts to record the decision. → Expect: c.outcome's self-approval branch fires, routing to h.rejected -- the request is never approved by its own requester.
- [authorized-approval] Given: A different, authorized reviewer than the requester records APPROVED. → Expect: The APPROVED branch's reviewer != requester condition is satisfied; a.approved fires normally.
- [reassignment] Given: The request is reassigned to a new reviewer partway through review. → Expect: The original requester identity is unchanged by reassignment, so the self-approval check still compares against the true original requester, not the current assignee.
- [duplicate-creation] Given: Two approval requests are attempted concurrently for the same subject version. → Expect: entity.concurrency "one-active-per-key" on instanceKey ["request_id"] plus a.create's idempotencyKey resolve the race to a single live request.
...
WHY:
The decision logic (irreversibility check, scoped-vs-full rollback capability) is precise and well-reasoned, and the journey's own text stresses this choice is irreversible if wrong ('a rollback across a migrated schema cannot be undone by rolling forward again'). a.determine now explicitly requires an authorized recovery-decision role to exercise this authority before rollback-vs-forward is decided, carries execution: 'human' with a declared 'task' channel, and an idempotencyKey scoped to the change plus the determination action — closing what was previously a P0 self-approval gap. The role is named structurally as an authorized recovery-decision role, not as an invented company-specific team.
...
INSTANCE:
Scope is the failing rollout/change and the targets it affects. The entity now declares instanceKey: ['change_id'] with concurrency: 'one-active-per-key', so two rollback determinations for the same change cannot run concurrently — the coherent decision unit is now backed by an explicit concurrency guard rather than an implied one.
...
AUTHORITY / APPROVAL:
a.determine now requires an authorized recovery-decision role to make the rollback-vs-forward call — an abstract structural role, not an invented company-specific team — closing the prior P0 gap where this high-stakes, direction-irreversible-if-wrong choice had no named sign-off.

IDEMPOTENCY:
a.determine now carries idempotencyKey: 'change_id + a.determine', scoped to the change and this specific determination action, guarding against a duplicate decision being recorded if the determination is retried or reassigned. Execution's own idempotency remains RLT-248's responsibility.
...
TEST CASES:
- [approval] Given: a change has crossed an irreversible boundary (schema migration) and the failure threshold is reached → Expect: rollback is refused (forward-only) and the boundary that was crossed is named — and a.determine cannot complete without an authorized recovery-decision role actually exercising that authority
- [self-approval] Given: the same automated process that detected the failure attempts to record a.determine's outcome with no role holder having acted → Expect: the determination does not complete — execution: 'human' means the decision requires the authorized recovery-decision role to act, not an automated pass-through
- [handoff] Given: the failure threshold is reached and a.determine is ready to be worked → Expect: the decision surfaces as an assignable human task on the declared 'task' channel, carrying idempotencyKey change_id + a.determine so a retried assignment does not record a second determination
...
WHY:
The evidentiary discipline (model output ≠ fact, monitoring ≠ restriction, proportionality) is exemplary, and the concurrent-case-creation race is now closed structurally rather than left to prose: entity declares instanceKey: ["risk_subject_id"] and concurrency: "one-active-per-key", and a.case is rewritten to atomic create-if-absent semantics carrying idempotencyKey: "risk_subject_id + a.case" — if no open case already exists for this risk_subject_id it creates one, and a concurrent evaluation that already opened one folds this evidence into the existing case rather than creating a second. A new guardrail states the invariant directly. A company mainly needs to wire the policy-version source, the decision-point call sites, and the per-risk_subject_id serialization at the data layer; no canonical logic gap blocks safe implementation.
...
INSTANCE:
instanceKey: ["risk_subject_id"], concurrency: "one-active-per-key" — now declared explicitly rather than implied. Identity is the specific evidence subject (the actor, account, transaction or resource the signal concerns, never the broader actor generally): one case per correlated risk, scoped to what the evidence is actually about (a flagged transaction stays a flagged transaction, not a flagged customer) — accumulating signals rather than opening a new case per signal, with a.case now the atomic authority for that identity.
...
IDEMPOTENCY:
Fixed: a.case now carries idempotencyKey: "risk_subject_id + a.case" and performs an atomic create-if-absent check — if no open case already exists for this risk_subject_id it creates one, and a concurrent evaluation that already created one folds this evidence into the existing case rather than opening a second. A new guardrail states the invariant directly: concurrent evaluation of the same risk_subject_id yields at most one canonical case. This closes what was previously the central defect — two concurrent signals about the same risk fragmenting into two cases that could reach contradictory conclusions.
...
TEST CASES:
- [duplicate-race] Given: two concurrent evaluations correlate to the same risk_subject_id before either creates a case → Expect: a.case's atomic create-if-absent check (idempotencyKey: risk_subject_id + a.case) guarantees exactly one case is created; the losing concurrent evaluator folds its evidence into the existing case rather than opening a second
...
WHY:
The single-use consumption discipline is explicit in prose ('any later invocation is refused rather than quietly honoured'), and the check-then-consume race behind it is now closed structurally rather than only asserted: entity declares instanceKey: ["exception_id"] and concurrency: "one-active-per-key", which serializes the entire check-then-act sequence (c.applicable → a.override → c.single → a.consume) per exception_id so two concurrent invocations cannot interleave through it. a.override carries idempotencyKey: "exception_id + invocation + a.override", and a.consume is rewritten to atomic consumption semantics with idempotencyKey: "exception_id + a.consume" — the first invocation to reach it claims the single use, and a concurrent second invocation finds it already consumed. A new guardrail states the invariant directly. No canonical logic gap remains; a company mainly needs to implement the per-exception_id serialization at the data layer.
...
INSTANCE:
instanceKey: ["exception_id"], concurrency: "one-active-per-key" — now declared explicitly. The entity note gives an unusually clear identity statement for this batch: 'The exception is one record; each use is an event on it. Validity is asked at each use rather than set once when it was granted' — and now states directly why the concurrency declaration exists: without serializing invocations against the same exception_id, two concurrent invocations of a single-use exception could both pass c.applicable's not-consumed check before either reached a.consume's mark, applying the override twice against an authorization for one.
...
IDEMPOTENCY:
Fixed: exception_id's concurrency: "one-active-per-key" now serializes the entire c.applicable → a.override → c.single → a.consume sequence per exception, so the three separate nodes can no longer interleave across concurrent invocations. a.override carries idempotencyKey: "exception_id + invocation + a.override" and a.consume carries idempotencyKey: "exception_id + a.consume", with a.consume's does-text rewritten to atomic consumption semantics: the first invocation to reach it claims the single use, and a concurrent second invocation finds it already consumed rather than applying a second override. A new guardrail states the invariant directly. This closes what was previously the central gap.
...
TEST CASES:
- [duplicate-race] Given: two concurrent invocations of a single-use exception arrive before either is marked consumed → Expect: exception_id's one-active-per-key concurrency serializes both invocations through c.applicable → a.override → c.single → a.consume; exactly one override applies and the second invocation finds the exception already consumed (a.consumed)
...
INSTANCE:
Work instance = one renewal execution operation and the prospective new term it would create, scoped explicitly as 'the renewal operation and the new term it would create' — entity.note is explicit that the new term is a genuinely new record, never overwriting the previous term's dates/price/scope. entity.instanceKey = ["renewal_cycle_id"], concurrency "one-active-per-key" — renewal_cycle_id is the same identity SUB-163 already tracks (idempotencyKey "renewal_cycle_id + relationship_id + a.decided"), so the two workflows now share one identity vocabulary rather than SUB-164 inventing a second name for the same thing.
...
IDEMPOTENCY:
Closed. a.financial now carries idempotencyKey "renewal_cycle_id + a.financial" and a.new-term carries idempotencyKey "renewal_cycle_id + a.new-term", both scoped to entity.instanceKey ["renewal_cycle_id"] with concurrency "one-active-per-key" — a duplicate renewal-authorized-for-execution event for the same renewal cycle can no longer raise a second financial obligation or create a second new term. SUB-163's own h.execute handoff now declares contract.requiredFields ["renewal_cycle_id"] and its carries text names the field explicitly, closing the receiver-construction gap this created.
...
TEST CASES:
- [duplicate-request] Given: SUB-163 sends a second renewal-authorized-for-execution event for the same renewal_cycle_id before the first completes. → Expect: entity.concurrency "one-active-per-key" plus a.financial's idempotencyKey resolve the duplicate to the same in-flight obligation rather than raising a second one; exactly one financial obligation and one new term result.
- [legitimate-separate-term] Given: A different, later renewal_cycle_id is authorized for the same relationship (a genuinely new renewal cycle, not a repeat of the first). → Expect: The distinct instanceKey means this is a separate instance from the first — its own a.financial/a.new-term proceed independently; the fix does not collapse legitimately separate renewal cycles into one.
...
WHY:
The consolidation logic itself is exemplary — per-state-type authority rules instead of blanket newest-wins, an explicit higher evidence bar for irreversible merges, and a thorough post-merge verification set. a.operation now explicitly requires an authorized identity-consolidation role to exercise this authority, carries execution: 'human' with a declared 'task' channel, and an idempotencyKey scoped to the merge operation and the action — closing what was previously a P0 gap where nobody was named as holding 'the authority' to authorize a merge. c.evidence's branch conditions now require an explicit elevated-bar exercise of that authority rather than inferring it from evidence quality alone. There is still no explicit correction/unmerge path for a merge later found wrong (tracked as a P1 below).
...
INSTANCE:
The merge operation is explicitly its own auditable entity — source IDs, canonical target, basis, authorizing authority, and every conflict decision. The entity now declares instanceKey: ['merge_operation_id'] with concurrency: 'one-active-per-key', so a single merge operation cannot be authorized or executed twice in parallel. Concurrency of two distinct merge operations touching overlapping source entities is still not addressed (see gaps).
...
AUTHORITY / APPROVAL:
a.operation now requires an authorized identity-consolidation role to authorize the merge, and t.authorized's evidence.requires names this role explicitly rather than leaving 'the authority' unspecified. c.evidence's branch conditions require an explicit elevated-bar exercise of that authority for the irreversible case, not just evidence meeting a bar unattended. The role is named structurally, not as an invented company-specific team, closing the prior P0 gap.

IDEMPOTENCY:
a.operation now carries idempotencyKey: 'merge_operation_id + a.operation', scoped to the merge operation and this specific action, guarding against a duplicate merge being recorded if authorization or execution is retried. Re-authorization of a wholly distinct merge operation for the same sources is not separately addressed.
...
TEST CASES:
- [self-approval] Given: the same automated process that produced a high match score also attempts to authorize the merge → Expect: the merge cannot complete — a.operation now requires an authorized identity-consolidation role to actually exercise that authority; a high match score alone does not satisfy it
- [approval] Given: an irreversible merge is proposed → Expect: evidence must meet the higher bar named for irreversible operations, and c.evidence's branch conditions now require an explicit elevated-bar exercise of the authorized identity-consolidation role, not just evidence quality alone
- [handoff] Given: a merge operation is authorized and ready for a.operation to run → Expect: the operation surfaces as an assignable human task on the declared 'task' channel, carrying idempotencyKey merge_operation_id + a.operation so a retried assignment does not record a duplicate operation
