...
/* Calculators index, in the calculator family's own language (the cream
   ...
   single-select chip row ABOVE the grid rather than a sidebar - the
   library is small enough that one active category is clearer (and was where
...
export type CalcEntry = {
  ...
  searchText: string;
  href: string;
  /** Short result cue used on the calculators index instead of repeating
      the category name the filter rail already shows. */
  cardMeta?: string;
};
...
const T = {
  en: {
    searchPlaceholder: "Search ROAS, CAC, LTV, retention, or UTM...",
    all: "All",
  ...
  return (
    ...
    >
      ...
      <span className="flex min-w-0 flex-1 flex-col sm:contents">
        ...
        <span className="mt-1 line-clamp-2 text-sm leading-relaxed text-ink-600 sm:mt-0">
          {entry.description}
        </span>
        <span className="mt-2 flex items-center justify-between gap-2 sm:mt-auto sm:pt-1.5">
          <span className="text-xs text-ink-subtle">{entry.cardMeta ?? entry.categoryLabel}</span>
          <ArrowRight
            aria-hidden
...
export function CalculatorLibrary({
  ...
  const results = useMemo(() => {
    ...
    return entries.filter((e) => {
      ...
      if (category && e.categoryLabel !== category) return false;
      return true;
    });
    ...
    <>
      ...
      {filtered && (
        <p aria-live="polite" className="mt-3 text-center text-sm text-ink-500">
          {t.countFiltered(resultCount, total)}
        </p>
      )}

      {/* The category facet is single-select: one active family at a time, plus
          an explicit All control that resets the search and category. Selected chips take the brand
          ...
          {categoryFacets.map((f) => (
            <div key={f.id} className="shrink-0 snap-start">
              <FacetChip
                label={f.label}
                count={f.count}
                checked={category === f.id}
                onToggle={() => setCategory(category === f.id ? null : f.id)}
              />
            </div>
          ))}
        </div>
