...
function Pill({ label, t }: { label: Label; t: (typeof copy)[Lang]["lab"]["page"]["practitioner"] }) {
  return <span className={`inline-block shrink-0 rounded px-1.5 py-0.5 font-mono text-[10px] font-medium tracking-wide whitespace-nowrap ${LABEL_PILL[label]}`}>{t.labels[label]}</span>;
}
...
function Step({ s, index, view, lang, t }: { s: TimelineStep; index: number; view: View; lang: Lang; t: (typeof copy)[Lang]["lab"]["page"]["practitioner"] }) {
...
export default function PractitionerView({ view, lang, t, basePath }: { view: View; lang: Lang; t: (typeof copy)[Lang]["lab"]["page"]["practitioner"]; basePath: string }) {
      ...
      <div className="mt-6 grid grid-cols-1 gap-x-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
        <div>
          ...
          <Block label={t.whoEnters}>
            <ul className="flex flex-col gap-1 text-[13px] leading-relaxed text-ink-700">{view.whoEnters.map((e) => <li key={e} className="flex gap-2"><span aria-hidden className="mt-2 size-1 shrink-0 rounded-full bg-ink-400" />{e}</li>)}</ul>
          </Block>
          <Block label={t.suppressedWhen}>
            <ul className="flex flex-col gap-1.5 text-[13px] leading-relaxed text-ink-700">{view.suppressedWhen.map((s) => <li key={s.id} className="flex items-start gap-2"><Pill label={s.label} t={t} /><span>{s.text}</span></li>)}</ul>
          </Block>
            ...
            <ul className="flex flex-col gap-2">
              {view.configure.map((c) => (
                <li key={c.key} className="text-[12.5px] leading-relaxed text-ink-700">
                  <span className="font-mono text-[12px] text-ink-900">{c.key}</span> <Pill label={c.label} t={t} />{c.class ? <span className="ml-1 font-mono text-[10px] text-ink-400">{c.class}</span> : null}
                  <br /><span className="font-mono text-[12px] text-ink-800">{c.short}</span> — {c.rule}
                </li>
              ))}
            </ul>
          </Block>
          <Block label={t.requiredData}>
            ...
            <ul className="mt-1 flex flex-col gap-0.5 text-[12.5px] text-ink-700">{view.requiredData.events.map((e) => <li key={e.id}><span className="font-mono text-[12px] text-ink-900">{e.id}</span> <span className="font-mono text-[10px] text-ink-400">{e.source}</span> — {e.meaning}</li>)}</ul>
        ...
        <div>
          ...
          <Block label={t.stopsWhen}>
            <ul className="flex flex-col gap-1 text-[13px] text-ink-700">
              ...
              {view.stopsWhen.handoffs.map((h) => <li key={h.id} className="flex gap-2"><span className="w-24 shrink-0 font-mono text-[11px] text-ink-400">{t.handoff}</span><span>{h.href ? <Link href={`${basePath}/${h.href}`} className="text-blue-600 hover:text-blue-700">{h.toName ?? h.to}</Link> : <span className="font-mono">{h.to}</span>} — {h.on}</span></li>)}
            </ul>
          </Block>
          ...
          <Block label={t.measurement}>
            <dl className="grid grid-cols-[8rem_1fr] gap-x-3 gap-y-1 text-[12.5px] text-ink-700">
              ...
              {bo ? (<>
                ...
                <dt className="text-ink-500">{t.attribution}</dt><dd>{bo.attribution} · {bo.comparison}{bo.holdout ? ` (${t.holdout} ${typeof bo.holdout.default?.value === "object" ? "" : String(bo.holdout.default?.value ?? "")}%)` : ""}</dd>
              </>) : null}
          ...
          {view.presets.length ? (
            <Block label={t.presets}>
              <ul className="flex flex-col gap-2 text-[12.5px] text-ink-700">{view.presets.map((p) => <li key={p.id}><span className="font-medium text-ink-900">{p.name}</span> <Pill label={p.applicableWhen.label} t={t} /> — {p.applicableWhen.text}{Object.keys(p.overrides).length ? <span className="block font-mono text-[11px] text-ink-500">{Object.entries(p.overrides).map(([k, v]) => `${k} = ${configValueText(v as never)}`).join(" · ")}</span> : null}</li>)}</ul>
            </Block>
          ) : null}
        </div>
      </div>
