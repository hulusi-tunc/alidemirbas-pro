...
export const STRUCTURE_JOURNEYS: readonly CanonicalJourney[] = [
  ...
  },
  {
    ...
    goal: "recovery-retry",
    channels: ["email", "push"],
    name: "Referral code issued → redeemed or unused → reward confirmed or one nudge",
    shortName: "Referral Reward",
    purpose:
      "Confirm the reward both sides earned the moment a referral code is redeemed, and send exactly one nudge back to sharing it if nobody has used it yet.",
    entity: {
      scope: "the person plus the referral code issued to them",
      note: "The code is the subject. A second code issued to the same person - after the first is used or lost - is its own instance.",
      instanceKey: [
        "person_id",
        "referral_code_id"
      ],
    ...
    eligibility: [
      "a referral or invite code generated for this person to share",
      "no instance of this journey is already open for the person plus this specific code",
      "hard gates (GLB-31) allow communication for this purpose"
    ],
    suppressions: [
      {
        "id": "s.g1",
        "label": "CANONICAL_RULE",
        "text": "A code issued is not a code used. Nothing about the reward is claimed until redemption is authoritatively recorded."
      },
      {
        "id": "s.g2",
        "label": "CANONICAL_RULE",
        "text": "One nudge, never two. A code nobody has used after the nudge stays unused rather than being asked about again."
      },
      {
        "id": "s.g3",
        "label": "CANONICAL_RULE",
        "text": "The reward named is the one actually credited, never a projected or typical amount."
      }
    ],
    contact: {
      ...
      "localCap": {
        "value": {
          "key": "referral_invite.touches",
          "rule": "One message per code: the reward confirmation if it was redeemed, or the one nudge if it wasn't - never both.",
          "default": {
            "value": 1,
            "confidence": "high",
            "basis": "corpus-rule",
            "applicableWhen": "the graph reaches exactly one communicating action per instance"
          },
          "required": false
        },
        "appliesTo": "all"
      ...
      "cooldown": {
        "key": "referral_invite.cooldown",
        "rule": "This journey is per the person plus the referral code issued to them; a new code is its own instance and no cooldown applies between them.",
        "default": {
          "value": "none",
          "confidence": "high",
          "basis": "corpus-rule",
      ...
      "roles": [
        {
          ...
          "when": "the reward confirmation, which has to be kept and survive until the person can act on it"
        },
        {
          "role": "low-friction",
          "channels": [
          ...
          "when": "the nudge back to sharing the code again, where a quick prompt is enough"
        }
      ],
      "fallback": "none",
      ...
      "touches": [
        {
          ...
          "purpose": "Tell them the reward both sides earned and how to use it.",
          "channelRoles": [
            "persistent"
          ],
          "mandatory": false,
        ...
        },
        {
          ...
          "purpose": "Remind them the code is unused and point them back to sharing it.",
          "channelRoles": [
            "low-friction"
          ],
          "mandatory": false,
    ...
    discovery: {
      ...
      "useCases": [
        "a referral code redeemed by a friend, reward confirmed to the referrer",
        "a referral code left unused after the redemption window, one nudge to share it again"
      ]
    },
    entry: "t.issued",
    nodes: [
      {
        id: "t.issued",
        kind: "trigger",
        event: "referral_code_issued",
        evidence: {
          requires: ["a referral or invite code generated for this person to share"],
          insufficientAlone: [
            "an intention to refer that has not produced a code",
            "a code issued to someone else",
          ],
          source: "authoritative",
        },
        next: "w.redeem",
      },
      {
        ...
        timeout: {
          "after": {
            "key": "referral_invite.redeem_window",
            "rule": "The window given for the code to be redeemed before a nudge is worth sending.",
            "class": "response-window",
            "default": {
              "value": "3 days",
              "confidence": "low",
              "basis": "example-only",
              "applicableWhen": "the referral program has no shorter promotional window of its own",
              "avoidWhen": "a time-limited campaign sets its own, shorter redemption window"
            },
            "required": false
          },
          "reason": "a code nobody has used is worth nudging about once, before it is forgotten entirely",
          "relativeTo": "trigger"
        },
        onTimeout: "c.redeemed",
        windowExtendsOnEngagement: false,
        recheck: "whether the code was redeemed, re-read from the system of record before acting on the timeout",
      },
      {
        id: "c.redeemed",
        kind: "condition",
        asks: "Was the code redeemed?",
        branches: [
          {
            label: "Redeemed",
            when: "a friend used the code",
            to: "a.reward",
          },
          {
            label: "Not yet",
            when: "nobody has used the code",
            to: "a.nudge",
          },
        ],
      },
      {
        id: "a.reward",
        kind: "action",
        does: "Tell them the reward both sides earned, the amount actually credited and how to use it",
        next: "x.rewarded",
        execution: "communication",
        idempotencyKey: "person_id + referral_code_id + a.reward",
      },
      {
        id: "x.rewarded",
        kind: "exit",
        state: "reward confirmed to the referring person after their code was redeemed",
        terminal: false,
        reEntry: "a new code issued to the same person opens its own instance",
        class: "success",
      },
      {
        id: "a.nudge",
        kind: "action",
        does: "Remind them the code is unused and point them back to sharing it, naming what they and a friend would both get",
        next: "x.unused",
        execution: "communication",
        idempotencyKey: "person_id + referral_code_id + a.nudge",
      },
      {
        id: "x.unused",
        kind: "exit",
        state: "code unused after the redemption window; nothing further is sent",
        terminal: false,
        reEntry: "the code being redeemed later re-opens this from the redemption event; a new code issued is its own instance",
        class: "timeout",
      },
    ],
    guardrails: [
      "A code issued is not a code used. Nothing about the reward is claimed until redemption is authoritatively recorded.",
      "One nudge, never two. A code nobody has used after the nudge stays unused rather than being asked about again.",
      "The reward named is the one actually credited, never a projected or typical amount.",
    ],
    reusableRule:
      "A referral code should be watched until it is redeemed or its window closes - confirm the reward the moment it is earned, and nudge exactly once if it isn't.",
  },
];
