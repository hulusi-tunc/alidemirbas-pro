...
const OVERRIDES: Readonly<Record<string, JourneyOverride>> = {
  ...
  purpose: "Sepetine ürün ekleyip checkout'a geçmeyen kişiyi, en yüksek değerli sepetlerde son adımda daha doğrudan bir kanala geçen üç adımlı bir hatırlatma dizisiyle geri getirmek; checkout başlar başlamaz Checkout Abandonment'a devretmek ve satın alma zaten gerçekleştiyse bir daha mesaj göndermemek.",
  nodes: {
  ...
  name: "Zorlanan kullanıcı tespiti → sebep sınıflandırması → kademeli yardım zinciri → kurtarma veya çıkış",
  ...
  nodes: {
    ...
    "w.tier2": { headline: "ürünün kendi kaydı takıldıkları adımı tamamlanmış olarak işaretleyene kadar", detail: "zaman aşımı: Push hatırlatmasının ardından kısa, sabit bir süre - zincir adımı yeniden okuyup e-posta ve canlı destek aşamasına geçmeden önce. (struggling_user.tier2_window üzerinden yapılandırılır)" },
    ...
    "w.tier3": { headline: "bu onboarding için yetkili aktivasyon olayı kaydedilene ya da bir destek talebi kaydedilene kadar", detail: "zaman aşımı: Mesaj ve canlı destek teklifinin ardından kısa, sabit bir süre - zincir tam onboarding tamamlanmasını yeniden okuyup örneği her iki yönde de kapatmadan önce. (struggling_user.tier3_window üzerinden yapılandırılır)" },
    ...
    "c.tier3-check": { headline: "Onboarding tamamlandı mı?", edges: [{ label: "Evet", detail: "bu onboarding için yetkili aktivasyon olayı kaydedildi" }, { label: "Hayır", detail: "onboarding hâlâ tamamlanmış olarak kaydedilmedi ve zincir sonuna geldi" }] },
    ...
    "x.declined": { headline: "zincir; tamamlama, canlı destek teması ya da daha sonra devam etme talebi olmadan sonuna kadar işledi", detail: "bekleme süresinden sonra yaşanan yeni bir zorlanma yeniden nitelik kazanabilir; zaten tüm zincirden geçmiş birine aynı zincir tekrar gönderilmez" },
  },
  },
  "ACT-16": {
  ...
  name: "Kayıp riski yükselmesi → katmanlı kanıt merdiveni → müdahale önceliği",
  purpose: "Bir kayıp riski sinyalini erken yakala ve kanıtın gerçekten hak ettiği kanalda ve öncelikte yanıtla, yalnızca kanıt kendini destekledikçe yükselterek.",
  nodes: {
    ...
    "w.high-wait": { headline: "2–3 gün bekle", detail: "zaman aşımı: Merdivenin en kısa sabit süresi - çünkü yüksek riskli bir ilişki bekletilecek bir ilişki değildir. (yapılandırma: churn_risk.high_tier_window)" },
    ...
    "c.final-outcome": { headline: "Kullanım ya da satın alma toparlandı mı?", edges: [{ label: "Düzeldi", detail: "kullanım ya da bir satın alma ölçülebilir şekilde toparlandı" }, { label: "Henüz değil", detail: "merdivenin tamamı boyunca hiçbir şey ölçülebilir şekilde toparlanmadı" }] },
    ...
    "x.passive-monitoring": { headline: "tüm yükselme merdiveni boyunca toparlanma olmadı; kayıp riski pasif takibe alındı", detail: "bu hesapta eşiğin yeniden aşılması, güncel kanıtlardan yeni bir bölüm açar" },
  },
  },
  "RET-27": {
