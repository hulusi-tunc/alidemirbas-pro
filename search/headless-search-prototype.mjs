...
/* ============================================================ pure engine
   Everything below takes its data as arguments - no fs, no import.meta.url,
   no module-load-time file reads. This is what makes the exact same
   ranking code usable both from plain Node scripts (which load data via fs,
   see the module-scope wiring further down) and from a Next.js route
   handler (which needs its data statically `import`-ed so Next's own build
   bundles it - a runtime fs.readFileSync of a path outside src/ is NOT
   reliably included in a Vercel serverless function's deployed file set,
   since Next's file-tracer can't see through a dynamically-constructed
   path.join(...) call to know it needs to bundle that JSON). */
export function createSearchEngine({ index, aliases, synonyms, ranking, taxonomy }) {
  /* -------------------------------------------------------- normalization */
  ...
  /* ----------------------------------------------------------- IDF weight
     ...
     search-query-fixtures.json ("75+ calculator tools" was initially
     outranking the one document that actually says "75+ calculator tools",
     Numerspace, purely because 43 other documents all contain the word
     "calculator"). */
  ...
  function search(query, { limit = 10 } = {}) {
    ...
    return {
      ...
      results: ranked.slice(0, limit).map((r) => ({ id: r.doc.id, type: r.doc.type, title: r.doc.title, url: r.doc.url, urlTr: r.doc.urlTr, summary: r.doc.summary, score: Number(r.score.toFixed(2)), explain: r.explain })),
    };
  }
